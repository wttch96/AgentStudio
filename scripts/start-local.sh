...
# 检测平台：Windows (Git Bash / MSYS) 下 venv 可执行文件在 Scripts/ 下
...
if [[ ! -f "${VENV_DIR}/.dependencies-ready" || "${BACKEND_DIR}/pyproject.toml" -nt "${VENV_DIR}/.dependencies-ready" ]]; then
  ...
  # 进入后端目录后使用相对路径 .[dev]，避免 Windows 下 Git Bash 路径不被 pip 识别
  (
    cd "${BACKEND_DIR}"
...
for _ in {1..40}; do
  if ! kill -0 "${BACKEND_PID}" 2>/dev/null; then
    echo "后端启动失败（日志：/backend.log）：" >&2
    tail -n 30 "${RUN_DIR}/backend.log" >&2
    exit 1
  fi
  if ! kill -0 "${FRONTEND_PID}" 2>/dev/null; then
    echo "前端启动失败（日志：/frontend.log）：" >&2
